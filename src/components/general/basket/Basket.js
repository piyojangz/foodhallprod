...
class Basket extends Component {
  ...
  componentWillReceiveProps(props) {
    ...
    if (props.setpayment) {
      ...
      this.setState({ paymenttype: props.type, paymentmethod: props.method, paymentindex: props.paymentindex });
  ...
  componentDidMount() {
    ...
    if (this.state.shop.shopdetail.manualshippingfee == '1') {
      this.setState({ caldeliveryfee: this.state.shop.shopdetail.deriveryfee });
    }
    else {
      this.getdeliveryfee();
  ...
  getdeliveryfee() {
    ...
    var params = { 
      latfrom: this.state.userdetail.lat,
      lngfrom: this.state.userdetail.lng,
      latto: this.state.shop.shopdetail.lat,
      lngto: this.state.shop.shopdetail.lng, 
    };
    ...
    fetch(AppConfig.api + 'api/getdeliveryfee', request).then((response) => { 
      ...
      .then((data) => {
        console.log(data); 
        this.setState({ caldeliveryfee: data.result });
      }).done();
  }
...
checkAmount = (newArray) => {
  ...
  total = summary + parseInt(this.state.caldeliveryfee);
...
sendOrder = (ordertype) => {
  if (this.state.paymentindex == 99) {
    this.alertmsg('กรุณาเลือกประเภทการชำระเงิน');
  }
  else {
      ...
      <View>
        <Row style={{ paddingTop: 20, }}>
          <Col style={{ alignItems: 'flex-start', justifyContent: 'center', paddingLeft: 8, }}>
            ...
            <Text style={{ color: '#464646', fontSize: 14, }}>{this.state.userdetail.name == 0 || this.state.userdetail.name == '' ? this.state.userdetail.email : this.state.userdetail.name}</Text>
          </Col>
      ...
      <KeyboardAvoidingView behavior='padding'  >
        <Grid>
          <Row style={{ height: 50, }}>
            ...
            <Col style={{ alignItems: 'flex-end', justifyContent: 'center', paddingRight: 8, }}><Text style={{ color: '#464646', fontWeight: 'bold', fontSize: 14, }}>{this.state.caldeliveryfee}฿</Text></Col>
          </Row>
          <Row style={{
          ...
          <TouchableOpacity onPress={() => Actions.setpayment({ shop: this.props.shopdetail, paymentindex: this.state.paymentindex })}>
            <Row style={{
              backgroundColor: '#F8E0E3',
              height: 50, borderBottomWidth: 1,
              borderColor: '#d7d7d7',
            }}>
              <Col style={{ alignItems: 'flex-start', justifyContent: 'center', paddingLeft: 8, }}><Text style={{ color: '#464646', fontSize: 14, }}>การชำระเงิน</Text></Col>
              <Col style={{ alignItems: 'flex-end', justifyContent: 'center', paddingRight: 28, }}><Text style={{ color: '#464646', fontWeight: 'bold', fontSize: 14, }}>{this.state.paymenttype}</Text></Col>
          ...
          <Row>
            <Col style={{ alignItems: 'flex-start', justifyContent: 'center', paddingLeft: 8, paddingTop: 10, }}>
