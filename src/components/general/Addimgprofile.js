...
/* Component ==================================================================== */
class Addimgprofile extends Component {
  ...
  save() {
    ...
    if (this.state.isimgchange == false) {
      ...
      fetch(AppConfig.api + 'api/upload_userimage', request).then((response) => {
        ...
        .then((data) => {
          ...
          Actions.pop({ refresh: { reload: true, image: data.input } });
        }).done();
...
export default connect(mapStateToProps, mapDispatchToProps)(Addimgprofile);
